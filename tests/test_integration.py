...
async def test_private_message_forwarding_flow():
    ...
    with patch('telegram_communa_bot.main.bot') as mock_bot:
        ...
        mock_bot.send_message.assert_called_once_with(
            123456,  # TARGET_GROUP_ID
            "[UID:TESTUID1] Hello, this is a test message"
        )
...
async def test_group_reply_forwarding_flow():
    ...
    with patch('telegram_communa_bot.main.bot') as mock_bot:
        ...
        mock_bot.send_message.assert_called_once_with(
            user_id,
            "Ответ из группы: This is a reply from group"
        )
...
async def test_unsupported_message_type():
    ...
    with patch('telegram_communa_bot.main.bot') as mock_bot:
        ...
        mock_message.answer.assert_called_once_with("Извините, этот тип сообщения не поддерживается.")
